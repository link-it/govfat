...
public class JDBCLottoFattureServiceImpl extends JDBCLottoFattureServiceSearchImpl
	...
	@Override
	public void create(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, LottoFatture lottoFatture) throws NotImplementedException,ServiceException,Exception {
		...
		sqlQueryObjectInsert.addInsertField(this.getLottoFattureFieldConverter().toColumn(LottoFatture.model().PROCESSATO,false),"?");
		...
		long id = jdbcUtilities.insertAndReturnGeneratedKey(sqlQueryObjectInsert, keyGenerator, jdbcProperties.isShowSql(),
			...
			new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(lottoFatture.getProcessato(),LottoFatture.model().PROCESSATO.getFieldType()),
	...
	@Override
	public void update(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdLotto oldId, LottoFatture lottoFatture) throws NotFoundException, NotImplementedException, ServiceException, Exception {
		...
		Long longId = lottoFatture.getId();
		if(longId != null && longId.longValue() > 0) {
			if(longId.longValue() != longIdByLogicId.longValue()) {
				throw new Exception("Ambiguous parameter: lottoFatture.id ["+longId+"] does not match logic id ["+longIdByLogicId+"]");
			}
		} else {
		...
		sqlQueryObjectUpdate.addUpdateField(this.getLottoFattureFieldConverter().toColumn(LottoFatture.model().PROCESSATO,false), "?");
		lstObjects_lottoFatture.add(new JDBCObject(lottoFatture.getProcessato(), LottoFatture.model().PROCESSATO.getFieldType()));
		...
		lstObjects_lottoFatture.add(new JDBCObject(longId, Long.class));
	...
	@Override
	public void updateOrCreate(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdLotto oldId, LottoFatture lottoFatture) throws NotImplementedException,ServiceException,Exception {
	
		if(this.exists(jdbcProperties, log, connection, sqlQueryObject, oldId)) {
			this.update(jdbcProperties, log, connection, sqlQueryObject, oldId, lottoFatture);
		} else {
			this.create(jdbcProperties, log, connection, sqlQueryObject, lottoFatture);
		}
