...
public class JDBCEnteServiceImpl extends JDBCEnteServiceSearchImpl
	...
	@Override
	public void create(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Ente ente) throws NotImplementedException,ServiceException,Exception {
		...
		sqlQueryObjectInsert.addInsertField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT,false),"?");
		sqlQueryObjectInsert.addInsertField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT_CONSEGNA_LOTTO,false),"?");
		sqlQueryObjectInsert.addInsertField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT_RICHIEDI_PROTOCOLLO,false),"?");
		...
		long id = jdbcUtilities.insertAndReturnGeneratedKey(sqlQueryObjectInsert, keyGenerator, jdbcProperties.isShowSql(),
			...
			new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(ente.getEndpoint(),Ente.model().ENDPOINT.getFieldType()),
			new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(ente.getEndpointConsegnaLotto(),Ente.model().ENDPOINT_CONSEGNA_LOTTO.getFieldType()),
			new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(ente.getEndpointRichiediProtocollo(),Ente.model().ENDPOINT_RICHIEDI_PROTOCOLLO.getFieldType())
		);
		ente.setId(id);
	...
	@Override
	public void update(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdEnte oldId, Ente ente) throws NotFoundException, NotImplementedException, ServiceException, Exception {
		...
		Long longId = ente.getId();
		if(longId != null && longId.longValue() > 0) {
			if(longId.longValue() != longIdByLogicId.longValue()) {
				throw new Exception("Ambiguous parameter: ente.id ["+longId+"] does not match logic id ["+longIdByLogicId+"]");
			}
		} else {
		...
		sqlQueryObjectUpdate.addUpdateField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT,false), "?");
		lstObjects_ente.add(new JDBCObject(ente.getEndpoint(), Ente.model().ENDPOINT.getFieldType()));
		sqlQueryObjectUpdate.addUpdateField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT_CONSEGNA_LOTTO,false), "?");
		lstObjects_ente.add(new JDBCObject(ente.getEndpointConsegnaLotto(), Ente.model().ENDPOINT_CONSEGNA_LOTTO.getFieldType()));
		sqlQueryObjectUpdate.addUpdateField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT_RICHIEDI_PROTOCOLLO,false), "?");
		lstObjects_ente.add(new JDBCObject(ente.getEndpointRichiediProtocollo(), Ente.model().ENDPOINT_RICHIEDI_PROTOCOLLO.getFieldType()));
		sqlQueryObjectUpdate.addWhereCondition("id=?");
		lstObjects_ente.add(new JDBCObject(longId, Long.class));
	...
	@Override
	public void updateOrCreate(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdEnte oldId, Ente ente) throws NotImplementedException,ServiceException,Exception {
	
		if(this.exists(jdbcProperties, log, connection, sqlQueryObject, oldId)) {
			this.update(jdbcProperties, log, connection, sqlQueryObject, oldId, ente);
		} else {
			this.create(jdbcProperties, log, connection, sqlQueryObject, ente);
		}
