...
import org.govmix.proxy.fatturapa.dao.jdbc.fetch.EnteFetch;
...
public class JDBCEnteServiceSearchImpl implements IJDBCServiceSearchWithId<Ente, IdEnte, JDBCServiceManager> {
	...
	@Override
	public Ente get(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdEnte id) throws NotFoundException, MultipleResultException, NotImplementedException, ServiceException,Exception {
		Long id_ente = this.findIdEnte(jdbcProperties, log, connection, sqlQueryObject, id, true);
		return this._get(jdbcProperties, log, connection, sqlQueryObject, id_ente);
	...
	@Override
	public List<IdEnte> findAllIds(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression) throws NotImplementedException, ServiceException,Exception {
		...
		// 2. Usare metodo getEnteFetch() sul risultato della select per ottenere un oggetto Ente
        ...
        for(Long id: ids) {
        	Ente ente = this.get(jdbcProperties, log, connection, sqlQueryObject, id);
	...
	@Override
	public List<Ente> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression) throws NotImplementedException, ServiceException,Exception {
	...
	@Override
	public Ente find(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCExpression expression) 
	...
	@Override
	public Ente get(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, long tableId) throws NotFoundException, MultipleResultException, NotImplementedException, ServiceException, Exception {
		return this._get(jdbcProperties, log, connection, sqlQueryObject, Long.valueOf(tableId));
	}
	
	private Ente _get(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Long tableId) throws NotFoundException, MultipleResultException, NotImplementedException, ServiceException, Exception {
		...
		// Object ente
		ISQLQueryObject sqlQueryObjectGet_ente = sqlQueryObjectGet.newSQLQueryObject();
		sqlQueryObjectGet_ente.setANDLogicOperator(true);
		sqlQueryObjectGet_ente.addFromTable(this.getEnteFieldConverter().toTable(Ente.model()));
		sqlQueryObjectGet_ente.addSelectField("id");
		sqlQueryObjectGet_ente.addSelectField(this.getEnteFieldConverter().toColumn(Ente.model().NOME,true));
		sqlQueryObjectGet_ente.addSelectField(this.getEnteFieldConverter().toColumn(Ente.model().DESCRIZIONE,true));
		sqlQueryObjectGet_ente.addSelectField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT,true));		
		sqlQueryObjectGet_ente.addSelectField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT_CONSEGNA_LOTTO,true));
		sqlQueryObjectGet_ente.addSelectField(this.getEnteFieldConverter().toColumn(Ente.model().ENDPOINT_RICHIEDI_PROTOCOLLO,true));

		sqlQueryObjectGet_ente.addWhereCondition("id=?");

		// Get ente
		ente = (Ente) jdbcUtilities.executeQuerySingleResult(sqlQueryObjectGet_ente.createSQLQuery(), jdbcProperties.isShowSql(), Ente.model(), this.getEnteFetch(),
	...
	private boolean _exists(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Long tableId) throws MultipleResultException, NotImplementedException, ServiceException, Exception {
		...
		sqlQueryObject.addSelectField(this.getEnteFieldConverter().toColumn(Ente.model().NOME,true));
	...
	protected Long findIdEnte(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdEnte id, boolean throwNotFound) throws NotFoundException, ServiceException, NotImplementedException, Exception {
		...
		sqlQueryObjectGet.addWhereCondition(this.getEnteFieldConverter().toColumn(Ente.model().NOME,true)+"=?");
