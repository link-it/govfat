...
public class JDBCNotificaDecorrenzaTerminiServiceImpl extends JDBCNotificaDecorrenzaTerminiServiceSearchImpl
	...
	@Override
	public void create(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, NotificaDecorrenzaTermini notificaDecorrenzaTermini) throws NotImplementedException,ServiceException,Exception {
	...
	@Override
	public void update(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdNotificaDecorrenzaTermini oldId, NotificaDecorrenzaTermini notificaDecorrenzaTermini) throws NotFoundException, NotImplementedException, ServiceException, Exception {
		...
		Long longId = notificaDecorrenzaTermini.getId();
		if(longId != null && longId.longValue() > 0) {
			if(longId.longValue() != longIdByLogicId.longValue()) {
				throw new Exception("Ambiguous parameter: notificaDecorrenzaTermini.id ["+longId+"] does not match logic id ["+longIdByLogicId+"]");
			}
		} else {
		...
		lstObjects_notificaDecorrenzaTermini.add(new JDBCObject(longId, Long.class));
	...
	@Override
	public void updateOrCreate(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdNotificaDecorrenzaTermini oldId, NotificaDecorrenzaTermini notificaDecorrenzaTermini) throws NotImplementedException,ServiceException,Exception {
	
		if(this.exists(jdbcProperties, log, connection, sqlQueryObject, oldId)) {
			this.update(jdbcProperties, log, connection, sqlQueryObject, oldId, notificaDecorrenzaTermini);
		} else {
			this.create(jdbcProperties, log, connection, sqlQueryObject, notificaDecorrenzaTermini);
		}
