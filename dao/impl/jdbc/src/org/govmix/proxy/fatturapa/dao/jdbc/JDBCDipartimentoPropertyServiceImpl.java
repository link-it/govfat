...
public class JDBCDipartimentoPropertyServiceImpl extends JDBCDipartimentoPropertyServiceSearchImpl
	implements IJDBCServiceCRUDWithId<DipartimentoProperty, IdProperty, JDBCServiceManager> {

	@Override
	public void create(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, DipartimentoProperty dipartimentoProperty) throws NotImplementedException,ServiceException,Exception {
		...
		// Object _ente
		Long id_ente = JDBCEnteUtils.toLongId(jdbcProperties, log, connection, sqlQueryObject, dipartimentoProperty.getIdEnte());
	...
	@Override
	public void update(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdProperty oldId, DipartimentoProperty dipartimentoProperty) throws NotFoundException, NotImplementedException, ServiceException, Exception {
		...
		Long longId = dipartimentoProperty.getId();
		if(longId != null && longId.longValue() > 0) {
			if(longId.longValue() != longIdByLogicId.longValue()) {
				throw new Exception("Ambiguous parameter: dipartimentoProperty.id ["+longId+"] does not match logic id ["+longIdByLogicId+"]");
			}
		} else {
		...
		lstObjects_dipartimentoProperty.add(new JDBCObject(longId, Long.class));
	...
	@Override
	public void updateOrCreate(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdProperty oldId, DipartimentoProperty dipartimentoProperty) throws NotImplementedException,ServiceException,Exception {
	
		if(this.exists(jdbcProperties, log, connection, sqlQueryObject, oldId)) {
			this.update(jdbcProperties, log, connection, sqlQueryObject, oldId, dipartimentoProperty);
		} else {
			this.create(jdbcProperties, log, connection, sqlQueryObject, dipartimentoProperty);
		}
	...
	public void delete(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, DipartimentoProperty dipartimentoProperty) throws NotImplementedException,ServiceException,Exception {
		...
		if( (dipartimentoProperty.getId()!=null) && (dipartimentoProperty.getId()>0) ){
			...
			IdProperty idDipartimentoProperty = this.convertToId(jdbcProperties,log,connection,sqlQueryObject,dipartimentoProperty);
	...
	@Override
	public void deleteById(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, IdProperty idDipartimentoProperty) throws NotImplementedException,ServiceException,Exception {
