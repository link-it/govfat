...
import org.govmix.proxy.fatturapa.dao.jdbc.fetch.UtenteFetch;
...
public class JDBCUtenteServiceSearchImpl implements IJDBCUtenteServiceSearch {
	...
	public List<Utente> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression) throws NotImplementedException, ServiceException,Exception {
		return findAll(jdbcProperties, log, connection, sqlQueryObject, expression,true);
	}
	...
	public List<Utente> findAll(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, JDBCPaginatedExpression expression, boolean caricaDipartimenti) throws NotImplementedException, ServiceException,Exception {
		...
		try {
			...
			fields.add(new AliasField(Utente.model().ENTE.NOME, "nomeEnte"));
			...
			for(Map<String, Object> map: returnMap) {
				...
				utente.setEnte((IdEnte)this.getUtenteFetch().fetch(jdbcProperties.getDatabase(), Utente.model().ENTE, map));
	...
	private void _mappingTableIds(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Utente obj, Utente imgSaved) throws NotFoundException,NotImplementedException,ServiceException,Exception{
		...
		if(obj.getUtenteDipartimentoList()!=null){
			List<org.govmix.proxy.fatturapa.UtenteDipartimento> listObj_ = obj.getUtenteDipartimentoList();
			for(org.govmix.proxy.fatturapa.UtenteDipartimento itemObj_ : listObj_){
				org.govmix.proxy.fatturapa.UtenteDipartimento itemAlreadySaved_ = null;
				if(imgSaved.getUtenteDipartimentoList()!=null){
					List<org.govmix.proxy.fatturapa.UtenteDipartimento> listImgSaved_ = imgSaved.getUtenteDipartimentoList();
					for(org.govmix.proxy.fatturapa.UtenteDipartimento itemImgSaved_ : listImgSaved_){
	...
	private Utente _get(JDBCServiceManagerProperties jdbcProperties, Logger log, Connection connection, ISQLQueryObject sqlQueryObject, Long tableId) throws NotFoundException, MultipleResultException, NotImplementedException, ServiceException, Exception {
		...
		// Object _utente_ente (recupero id)
		ISQLQueryObject sqlQueryObjectGet_utente_ente_readFkId = sqlQueryObjectGet.newSQLQueryObject();
		sqlQueryObjectGet_utente_ente_readFkId.addFromTable("utenti");
		sqlQueryObjectGet_utente_ente_readFkId.addSelectField("id_ente");
		sqlQueryObjectGet_utente_ente_readFkId.addWhereCondition("id=?");
		sqlQueryObjectGet_utente_ente_readFkId.setANDLogicOperator(true);
		Long idFK_utente_ente = (Long) jdbcUtilities.executeQuerySingleResult(sqlQueryObjectGet_utente_ente_readFkId.createSQLQuery(), jdbcProperties.isShowSql(),Long.class,
				new JDBCObject(utente.getId(),Long.class));

		utente.setEnte(JDBCEnteUtils.toLogicId(jdbcProperties, log, connection, sqlQueryObject, idFK_utente_ente));
		...
		if(utente_utenteDipartimento_list != null) {
			for (Object utente_utenteDipartimento_object: utente_utenteDipartimento_list) {
				...
				sqlQueryObjectGet_utente_utenteDipartimento_dipartimento_readFkId.addFromTable(this.getUtenteFieldConverter().toTable(Utente.model().UTENTE_DIPARTIMENTO));
				...
				DipartimentoFieldConverter dipFC = new DipartimentoFieldConverter(jdbcProperties.getDatabaseType());

				// Object _utente_utenteDipartimento_dipartimento
				ISQLQueryObject sqlQueryObjectGet_utente_utenteDipartimento_dipartimento = sqlQueryObjectGet.newSQLQueryObject();

				sqlQueryObjectGet_utente_utenteDipartimento_dipartimento.addFromTable(dipFC.toTable(Dipartimento.model()));
				sqlQueryObjectGet_utente_utenteDipartimento_dipartimento.addSelectField(dipFC.toColumn(Dipartimento.model().CODICE, false));
				//...
				sqlQueryObjectGet_utente_utenteDipartimento_dipartimento.setANDLogicOperator(true);
				sqlQueryObjectGet_utente_utenteDipartimento_dipartimento.addWhereCondition("id=?");

				// Recupero _utente_utenteDipartimento_dipartimento
				org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject [] searchParams_utente_utenteDipartimento_dipartimento = new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject [] { 
						new org.openspcoop2.generic_project.dao.jdbc.utils.JDBCObject(idFK_utente_utenteDipartimento_dipartimento,Long.class)
				};
				List<Class<?>> listaFieldIdReturnType_utente_utenteDipartimento_dipartimento = new ArrayList<Class<?>>();
				listaFieldIdReturnType_utente_utenteDipartimento_dipartimento.add(Dipartimento.model().CODICE.getFieldType());

				List<Object> listaFieldId_utente_utenteDipartimento_dipartimento = jdbcUtilities.executeQuerySingleResult(sqlQueryObjectGet_utente_utenteDipartimento_dipartimento.createSQLQuery(), jdbcProperties.isShowSql(),
						listaFieldIdReturnType_utente_utenteDipartimento_dipartimento, searchParams_utente_utenteDipartimento_dipartimento);
				// set _utente_utenteDipartimento_dipartimento

				IdDipartimento id_utente_utenteDipartimento_dipartimento = new IdDipartimento();

				id_utente_utenteDipartimento_dipartimento.setCodice((String) listaFieldId_utente_utenteDipartimento_dipartimento.get(0));
	...
	private void _join(IExpression expression, ISQLQueryObject sqlQueryObject) throws NotImplementedException, ServiceException, Exception{
		...
		if(expression.inUseModel(Utente.model().ENTE,false)){
			String tableName1 = this.getUtenteFieldConverter().toAliasTable(Utente.model());
			String tableName2 = this.getUtenteFieldConverter().toAliasTable(Utente.model().ENTE);
