		...
		try{
			if(detached){
				...
				if(this.idEsito != null)
					this.getDataProvider().setIdEsito(this.idEsito);
				List<ErroreElaborazionePccBean> list = this.getDataProvider().findAll(start, limit);
				for (ErroreElaborazionePccBean traccia : list) {
