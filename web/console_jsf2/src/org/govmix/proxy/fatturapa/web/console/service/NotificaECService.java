 ...
 *
 */
@RequestScoped @ManagedBean(name="notificaECService")
public class NotificaECService extends BaseService<NotificaECForm> implements INotificaECService{
	...
	private org.govmix.proxy.fatturapa.model.IdFatturaModel idFatturaModel = null;
	...
	public NotificaECService(){
		try{
			...
			this.notificaDAO = DAOFactory.getInstance(log).getServiceManager().getNotificaEsitoCommittenteService();
			this.notificaSearchDAO = DAOFactory.getInstance(log).getServiceManager().getNotificaEsitoCommittenteServiceSearch();
		}catch(Exception e){
	...
	public List<NotificaECBean> findAll(int start, int limit)
		...
		try {
			...
			IExpression expr = getFiltroRicerca(this.idFattura,this.notificaSearchDAO, this.idFatturaModel,log);
			...
			log.error("Si e' verificato un errore durante la find all delle notifiche:" + e.getMessage(), e);
			throw e;
	...
	public int totalCount() throws ServiceException {
		...
		try {
			...
			log.error("Si e' verificato un errore durante la count delle notifiche:" + e.getMessage(), e);
			throw e;
		}  catch (NotImplementedException e) {
			log.error("Si e' verificato un errore durante la count delle notifiche:" + e.getMessage(), e);
			throw new ServiceException(e);
	...
	public void store(NotificaECBean obj) throws ServiceException {
		...
		try {
			...
			log.error("Si e' verificato un errore durante l'esecuzione della store della notifica:" + e.getMessage(), e);
			throw new ServiceException(e);
		} catch (ServiceException e) {
			log.error("Si e' verificato un errore durante l'esecuzione della store della notifica:" + e.getMessage(), e);
			throw e;
	...
	public NotificaECBean findById(Long key)  throws ServiceException {
		...
		try {
			...
			log.debug("[FindbyId] NotificaEC Non trovata: "+ e.getMessage());
			throw new ServiceException(e);
		}  catch (MultipleResultException e) {
			log.error("Si e' verificato un errore durante l'esecuzione della find by id ["+key+"]:" + e.getMessage(), e);
			throw new ServiceException(e);
		} catch (NotImplementedException e) {
			log.error("Si e' verificato un errore durante l'esecuzione della find by id ["+key+"]:" + e.getMessage(), e);
			throw new ServiceException(e);
	...
	public static IExpression getFiltroRicerca(IdFattura idFattura, IServiceSearchWithoutId<?> searchDAO, org.govmix.proxy.fatturapa.model.IdFatturaModel idFatturaModel, Logger log) throws ServiceException {
