...
if (!A4J.findForm) {
	function _JSFFormSubmit(linkId, formName, target, parameters) {
		var form = (typeof formName == 'string' ? document
				.getElementById(formName) : formName);
		if (form) {
			var paramNames = [];
			...
			if (parameters) {
				for ( var param in parameters) {
					...
					if (form.elements[param]) {
						...
						if (param === "javax.faces.portletbridge.STATE_ID"
								&& form.firstChild) {
	...
	A4J.setupForm = function(id) {
		var element = (typeof id == 'string' ? window.document
