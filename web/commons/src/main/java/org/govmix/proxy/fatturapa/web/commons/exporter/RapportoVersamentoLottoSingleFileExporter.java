...
public class RapportoVersamentoLottoSingleFileExporter extends AbstractSingleFileExporter<FatturaElettronica, IdFattura>{
	
	private ISIPService sipDAO;

	public RapportoVersamentoLottoSingleFileExporter(Logger log) throws ServiceException, NotImplementedException, Exception {
		super(log);
		this.sipDAO = DAOFactory.getInstance().getServiceManager().getSIPService();
	}

	public RapportoVersamentoLottoSingleFileExporter(Logger log, Connection connection, boolean autocommit) throws ServiceException, NotImplementedException, Exception {
	...
	public byte[] getRawContent(FatturaElettronica object) {
		try {
			SIP sip = this.sipDAO.get(object.getLottoFatture().getIdSIP());
